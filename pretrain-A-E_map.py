...
try:
    ...
    for i_epoch in range(c.epochs): # 迭代次数
        ...
        vgg.to(device)
        for i_batch, (cover, secret_1, secret_2) in enumerate(trainloader): # 一个batch的样本一起计算loss，然后更新参数
            # data preparation
            ...
            ## perceptual loss
            perc_loss_1 = guide_loss(vgg_on_cov, vgg_on_steg1)
            perc_loss_2 = guide_loss(vgg_on_cov, vgg_on_steg2)
            ...
            ## total loss
            total_loss_1 = c.lamda_reconstruction_1 * r_loss_1 + c.lamda_guide_1 * g_loss_1 + c.lamda_low_frequency_1 * l_loss_1 + 0.05 * perc_loss_1
            total_loss_2 = c.lamda_reconstruction_2 * r_loss_2 + c.lamda_guide_2 * g_loss_2 + c.lamda_low_frequency_2 * l_loss_2 + 0.05 * perc_loss_2
            ## 参数更新
            ...
            loss_history.append([(total_loss_1+total_loss_2).item(), 0.])
        ...
        if i_epoch % c.val_freq == 0:  # 修改？？
            with torch.no_grad():
                ...
                for cover, secret_1, secret_2 in testloader:
                    ...
                    secret_2_255 = ldr_denorm(tensor_to_numpy(secret_2).squeeze())  # (H,W,3)
                ...
                f.write(
                    ...
                    + "C1 average psnr: " + str(np.mean(psnr_c1_hdr)) + ", "
                    + "S2 average psnr: " + str(np.mean(psnr_s2)) + ", "
                    + "C2 average psnr: " + str(np.mean(psnr_c2_hdr)) + "\n"

                )
                print(
                    "[test]  S1 average psnr: %.2f, C1 average psnr: %.2f, S2 average psnr: %.2f, C2 average psnr:  %.2f" % (
                    np.mean(psnr_s1), np.mean(psnr_c1_hdr), np.mean(psnr_s2), np.mean(psnr_c2_hdr)))
        ...
        print(
            "[train]  lr: %.4f, Train_Loss: %.4f, concealing_loss_1: %.4f, concealing_loss_2: %.4f, revealing_loss_1: %.4f, revealing_loss_2: %.4f" % (
        ...
        if i_epoch > 0 and (i_epoch % c.SAVE_freq) == 0:
            torch.save({'opt': optim1.state_dict(),
                        'net': net1.state_dict()}, c.MODEL_PATH + 'pre_model_checkpoint_%.5i_1' % i_epoch + '.pt')
            torch.save({'opt': optim2.state_dict(),
                        'net': net2.state_dict()}, c.MODEL_PATH + 'pre_model_checkpoint_%.5i_2' % i_epoch + '.pt')
    ...
    torch.save({'opt': optim1.state_dict(),
                'net': net1.state_dict()}, c.MODEL_PATH + 'pre_model_1' + '.pt')
    torch.save({'opt': optim2.state_dict(),
                'net': net2.state_dict()}, c.MODEL_PATH + 'pre_model_2' + '.pt')
    ...
    if c.checkpoint_on_error:
        torch.save({'opt': optim1.state_dict(),
                    'net': net1.state_dict()}, c.MODEL_PATH + 'pre_model_ABORT_1' + '.pt')
        torch.save({'opt': optim2.state_dict(),
                    'net': net2.state_dict()}, c.MODEL_PATH + 'pre_model_ABORT_2' + '.pt')
