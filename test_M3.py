...
net1 = Model_1()  # 第一个IHNN
net2 = Model_1()    # 第二个IHNN
net1.cuda()
net2.cuda()
...
with torch.no_grad():
    ...
    for i, (cover, secret_1, secret_2) in enumerate(testloader):
        ...
        input_dwt_2 = torch.cat((output_steg_dwt_1, secret_dwt_2), 1)
        ...
        # resi_steg_1 = np.abs(cover_hdr - steg_1_hdr)
        # resi_steg_1 = resi_steg_1 * 7
        # resi_steg_2 = np.abs(cover_hdr - steg_2_hdr)
        # resi_steg_2 = resi_steg_2 * 7
        # resi_steg_1_ldr = tonemapReinhard.process(resi_steg_1)
        # resi_steg_2_ldr = tonemapReinhard.process(resi_steg_2)
        # cv2.imwrite(c.TEST_PATH_resi_steg_1 + '%.5d.png' % i, resi_steg_1_ldr[..., ::-1] * 255)
        # cv2.imwrite(c.TEST_PATH_resi_steg_2 + '%.5d.png' % i, resi_steg_1_ldr[..., ::-1] * 255)
