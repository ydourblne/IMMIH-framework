...
class PrepareBlock(nn.Module):
    def __init__(self, input, bias=True):
        ...
        self.lrelu = nn.LeakyReLU(inplace=True)
        self.senet = SELayer(channel=input + 2 * 64)  # attension模块？？
...
class ImpMapBlock(nn.Module):
    def __init__(self):
        ...
        self.rrdb = ResidualDenseBlock_out(input=64 + 64 + 64, output=3)    # 生成dense layers，输出3通道的importance map
    ...
    def forward(self, cover, secret, steg):  # 输入为前一层的cover,secret,stego

        x_cover = self.s_cover(cover)
        x_steg = self.s_steg(steg)
        x_secret = self.s_secret(secret)
        x = torch.cat((x_cover, x_steg), 1)
        x = torch.cat((x, x_secret), 1) # 把三个拼接起来
        x = self.rrdb(x)        # 经过dense layers
