...
try:
    ...
    for i_epoch in range(c.epochs): # 迭代次数
        ...
        for i_batch, (cover, secret_1, secret_2) in enumerate(pbar): # 一个batch的样本一起计算loss，然后更新参数
            ...
            input_dwt_2 = torch.cat((output_steg_dwt_1, secret_dwt_2), 1) #24

            output_dwt_2 = net2(input_dwt_2)  # channels = 24
            ...
            output_z_guass_2 = gauss_noise(output_z_dwt_2.shape)  # channels = 12

            output_rev_dwt_2 = torch.cat((output_steg_dwt_2, output_z_guass_2), 1)  # channels = 24

            rev_dwt_2 = net2(output_rev_dwt_2, rev=True)  # channels = 24
